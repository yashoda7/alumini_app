import 'package:alumni_app/screens/resources/resource_upload_screen.dart';
          ...
          if (canUpload)
            IconButton(
              onPressed: () => Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => ResourceUploadScreen(
...
class _ResourceDetailScreenState extends State<ResourceDetailScreen> {
  ...
  void initState() {
    super.initState();
    widget.firestoreService.incrementResourceViews(widget.resource.id);
  }
        ...
        child: Column(
          ...
          children: [
            ...
            StreamBuilder<List<ResourceReview>>(
              stream: widget.firestoreService.watchResourceReviews(widget.resource.id),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator());
                }
...
void _downloadResource() async {
  ...
  try {
    ...
    widget.firestoreService.incrementResourceDownloads(widget.resource.id);
          ...
          ElevatedButton(
            onPressed: () {
              ...
              final review = ResourceReview(
                ...
                reviewerName: widget.currentUser['name'] ?? 'User',
                rating: rating.clamp(1, 5),
                comment: commentController.text.trim(),
                createdAt: DateTime.now(),
              );
              widget.firestoreService.createResourceReview(review);
